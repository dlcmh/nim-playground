...
proc score*(dice: array[5, int], category: Category): int =
  ...
  if category in Ones .. Sixes:
    result = category.ord * counts[category.ord]
  elif category == FullHouse and counts.largest[1] == 3:
    for k, v in counts.pairs:
      result += k * v
  elif category == FourOfAKind and counts.largest[1] >= 4:
    result = counts.largest[0] * 4
  elif category == LittleStraight and dice.sorted == [1, 2, 3, 4, 5]:
    result = 30
  elif category == BigStraight and dice.sorted == [2, 3, 4, 5, 6]:
    result = 30
  elif category == Yacht and counts.largest[1] == 5:
    result = 50
