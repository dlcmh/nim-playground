...
proc meetup*(year: int, month: int, schedule: Schedule, day: WeekDay): string =
  ...
  if schedule == Last:
    dt = dateTime(year, month, getDaysInMonth(month, year))
    ...
    while dt.weekday != day:
      dt += 1.days
    if schedule in [Second, Third, Fourth]:
      dt += (schedule.ord - 1).weeks
  dt.format("yyyy-MM-dd")

proc meetupV2*(year: int, month: int, schedule: Schedule,
    day: WeekDay): string =
  # let monthName = cast[Month](month)
  let monthName = Month(month)
  let daysInMonth = getDaysInMonth(monthName, year)
  var dt = dateTime(year, monthName, 1)
  while dt.weekday != day:
    dt += 1.days
  if schedule != First:
    for i, v in countup(dt.monthday + 7, daysInMonth, 7).toSeq:
      dt += (v - dt.monthday).days
      if schedule == Second and i == 0: break
      if schedule == Third and i == 1: break
      if schedule == Fourth and i == 2: break
      if schedule == Teenth and dt.monthday in 13 .. 19: break
      if schedule == Last: continue
  dt.format("yyyy-MM-dd")

proc meetupV1*(year: int, month: int, schedule: Schedule,
    day: WeekDay): string =
  let monthName = cast[Month](month)
  var dt = dateTime(year, monthName, 1)
  while dt.weekday != day:
  ...
  if schedule == Fourth:
    dt += 3.weeks
  if schedule == Teenth:
    while dt.monthday notin 13 .. 19:
      dt += 1.weeks
  if schedule == Last:
    while dt.month == monthName:
      dt += 1.weeks
  dt.format("yyyy-MM-dd")

when isMainModule:
  # echo meetup(2013, 5, Teenth, dMon) # "2013-05-13"
  # echo meetup(2013, 3, First, dMon) # "2013-03-04"
  # echo meetup(2013, 3, Second, dMon) # "2013-03-11"
  # echo meetup(2013, 3, Third, dMon) # "2013-03-18"
  # echo meetup(2013, 3, Last, dMon) # "2013-03-25"
